...
if (@$_GET['action'] == 'post') {
	...
	if (!!$_rows = _fetch_array("SELECT tg_uniqid
		...
		//写入数据库
		_query("INSERT INTO tg_article (tg_username,tg_title,tg_type,tg_content,tg_date) 
											VALUES ('{$_clean['username']}',
													'{$_clean['title']}',
													'{$_clean['type']}',
													'{$_clean['content']}',
													NOW())
...
<div id="post">
	...
	<form method="post" name="post" action="?action=post">
		<dl>
			...
			<dd>
				<?php 
					include ROOT_PATH.'includes/ubb.inc.php';
				?>
				<textarea name="content" rows="9"></textarea>
			</dd>
			<dd>验证码&nbsp;:<input type="text" name="code"class="text yzm" />&nbsp;&nbsp;<img src="code.php" id="code"></img> <input type="submit" class="submit" value="发表帖子" /></dd>
		</dl>
	</form>
</div>
